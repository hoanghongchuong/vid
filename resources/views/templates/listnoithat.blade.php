	...
	<section class="about">
		...
		<div class="container">
			<ul class="s14 text-uppercase justify-content-xl-center nav nav-pills da-tabs" role="tablist">
			  ...
			  <li class="nav-item">
			    <a class="nav-link @if($category->id == $cate->id) active @endif" href="{{url('noi-that/'.$category->alias)}}">{{$category->name}}</a>
			  </li>
			  @endforeach
			</ul>
						...
						<div class="col-sm-6">
							<article class="kt-item">
								<figure class="text-center da-item-img ">
									<a href="{{ url('noi-that/'.$item->alias.'.html') }}" title="{{$item->name}}"><img src="{{asset('upload/news/'.$item->photo)}}" title="{{$item->name}}" alt="{{$item->name}}"></a>
								</figure>
								<figcaption class="kt-item-info">
									<h2 class="text-uppercase py-3 kt-item-tit"><a href="{{ url('noi-that/'.$item->alias.'.html') }}" title="{{$item->name}}">{{$item->name}}</a></h2>
									<div class="t1 kt-content">
										{!! $item->mota !!}
									</div>

									<h3 class="text-lg-left text-center italic t1 kt-link"><a href="{{ url('noi-that/'.$item->alias.'.html') }}" title="">Xem chi tiết</a></h3>
								</figcaption>
							</article>
						</div>
						@endforeach
